...
![Resistor color code chart showing band sequences for 220 Ω and 10 kΩ](images/01-220-ohm-resistor.jpeg)
...
![LED polarity diagram showing anode (long leg) and cathode (short leg) identification](images/02.led-anode-cathode.jpg)
...
![Potentiometer pinout diagram showing GND (left), wiper/signal (middle), and 5V (right) pins](images/03.potentiometer-pinout.jpg)
...
![LM35 TO-92 package pinout — flat face view showing VCC (left), VOUT (middle), GND (right)](images/04.lm35-pinout.png)
...
![Push button 4-pin layout showing internal connections and correct breadboard placement](images/05.push-button-switch1-550x550.jpg)
...
![Arduino IDE 2.0 interface with Verify, Upload, Serial Monitor, board selector, port selector, and error panel labeled](images/06-arduino-ide-2-overview.webp)
...
![Arduino UNO R3 board diagram with all pin regions labeled — power pins, digital pins, PWM pins, and analog pins](images/07-Arduino-Uno-Pin-Diagram-picture.jpg)

![Arduino UNO detailed pinout showing all pin functions and technical specifications](images/08-Arduino-UNO-Pinout-Diagram-detailed-technical.jpg)
...
![Digital vs analog signal comparison — digital pin sees only HIGH/LOW; analog pin measures the full continuous range](images/09-Digital-Vs-Analg-signal.webp)
...
void setup() {
  // Serial communication must be initialized before any Serial.print() will work.
  // The baud rate here must match the dropdown in the Serial Monitor (set it to 9600).
  // The function is Serial.begin() and it takes the baud rate as its argument.
  // ▶ Write the line to initialize serial communication at 9600 baud:


  // Print a title and separator so the output is easy to identify.
  // These lines are already written for you — do not change them.
...
void loop() {
  // analogRead() reads the voltage at an analog pin and returns an integer from 0 to 1023.
  // We want to read pin A0, which has nothing connected to it.
  // Declare an int variable named 'reading' and store the result of analogRead(A0) in it.
  // ▶ Write the line to read A0 and store it in a variable called reading:


  // Now print the label "A0: " followed by the value of reading, then end the line.
  // Use Serial.print() for the label and Serial.println() for the value.
  // ▶ Write the two print lines below:


  // Wait 300 milliseconds before the next reading so the output scrolls at a readable pace.
  // The function is delay() and it takes milliseconds as its argument.
...
// It is good practice to declare pin numbers as named constants at the top of the sketch.
// This way, if the pin ever changes, you only update it in one place.
// 'const int' means the value cannot be changed after it is set.
// ▶ Declare a constant integer named LED_PIN and assign it the value 13:


void setup() {
  // A digital pin must be configured as OUTPUT before you can drive it with digitalWrite().
  // Without this, the pin stays in INPUT mode by default and the LED will not respond.
  // Use pinMode() with LED_PIN and OUTPUT as arguments.
  // ▶ Write the line to configure LED_PIN as an output:


  // Initialize serial so we can print messages to the Serial Monitor.
  // ▶ Write the line to begin serial at 9600 baud:
...
void loop() {
  // Drive LED_PIN HIGH (5V) to turn the LED on.
  // Use digitalWrite() with LED_PIN and HIGH as arguments.
  // ▶ Write the line to turn the LED on:


  // Print "LED: ON" to the Serial Monitor so we can track the state in text.
  Serial.println("LED: ON");

  // Wait for 1000 milliseconds (1 second) so the LED stays on long enough to see.
  // ▶ Write the delay line for 1000 ms:


  // Drive LED_PIN LOW (0V) to turn the LED off.
  // ▶ Write the line to turn the LED off:


  // Print "LED: OFF" to the Serial Monitor.
  // ▶ Write the print line:


  // Wait another 1000 ms before the loop repeats and turns the LED on again.
  // ▶ Write the second delay line:
...
![Potentiometer internal structure showing the resistive track, wiper contact, and voltage output at different shaft positions](images/10-potentiometer-internal-connection.jpg)
...
![Potentiometer wiring on breadboard — left pin to GND, right pin to 5V, middle wiper to Arduino A0](images/11.%20Potentiometer%20wiring%20diagram%20(breadboard).png)
...
// Declare the potentiometer pin as a named constant.
// This makes the code easier to read and change later.
// ▶ Declare a constant integer named POT_PIN and assign it the value A0:
...
void setup() {
  ...
  // Print column headers so the output is formatted and easy to read.
  // These lines are already written for you.
...
void loop() {
  // Read the wiper voltage from the potentiometer.
  // analogRead() returns an integer between 0 and 1023.
  // Store the result in an int variable named rawValue.
  // ▶ Write the analogRead line:


  // Convert the raw ADC value to voltage using the formula: V = ADC × (5.0 / 1023.0)
  // Use a float variable named voltage.
  // Remember: use 5.0 and 1023.0 (not 5 and 1023) to force floating-point division.
  // ▶ Write the voltage conversion line:


  // Print the ADC value and voltage on one line, separated by " | "
  // The formatting lines below are partially written — fill in the missing prints.
  Serial.print("     ");
  // ▶ Print rawValue (no newline):

  Serial.print("       |     ");
  // ▶ Print voltage with 3 decimal places (no newline):

  // ▶ End the line by printing " V" with a newline:


  // Wait 300 ms before the next reading — fast enough to feel responsive while turning the knob.
  // ▶ Write the delay line:
...
// ▶ Declare a constant integer named POT_PIN and assign it A0:
...
void setup() {
  ...
  // Print the column headers — already written for you.
...
void loop() {
  // ▶ Read from POT_PIN and store the result in an int named rawValue:


  // Map the ADC range (0–1023) to a percentage (0–100).
  // map() takes five arguments: the value, input low, input high, output low, output high.
  // Store the result in an int named percent.
  // ▶ Write the map line for percentage:


  // Map the ADC range (0–1023) to degrees (0–270).
  // Most potentiometers rotate 270 degrees, so this gives the physical shaft angle.
  // Store the result in an int named angle.
  // ▶ Write the map line for angle:


  // Print all three values on one line.
  // The formatting is partially written — fill in the missing prints.
  Serial.print("  ");
  // ▶ Print rawValue (no newline):

  Serial.print("   |    ");
  // ▶ Print percent (no newline):

  Serial.print("%     |    ");
  // ▶ Print angle (no newline):

  // ▶ End the line by printing "°" with a newline:
...
![LED circuit schematic — Arduino Pin 9 → 220 Ω resistor → LED anode → LED cathode → GND, with voltage drops labeled](images/12.%20LED%20circuit%20diagram%20(schematic%20style).png)
...
![PWM duty cycle timing diagram showing 25%, 50%, 75%, and 100% duty cycles with corresponding LED brightness](images/13-a-pwm%20duty%20cycle%2025%2050%2075%20100%20percent.jpeg)

![Animated illustration of PWM duty cycle varying from 0% to 100%](images/13-b-pwm-gif.gif)
...
![LED and potentiometer wiring on breadboard — potentiometer on A0, LED with 220 Ω resistor on Pin 9](images/14-pwm-gif.gif)
...
// Declare the LED pin as a named constant.
// We use pin 9 because it supports PWM (~) — needed for Task 3.2.
// ▶ Declare a constant integer named LED_PIN and assign it the value 9:


void setup() {
  // A digital pin must be set as OUTPUT before digitalWrite() will control it.
  // Without this, the pin stays in INPUT mode by default.
  // ▶ Write the line to configure LED_PIN as an output using pinMode():
...
void loop() {
  // Drive LED_PIN to HIGH (5V) — this turns the LED on.
  // ▶ Write the digitalWrite line to set LED_PIN HIGH:


  Serial.println("LED: ON");

  // Wait 1000 ms so the LED stays on long enough to see clearly.
  // ▶ Write the delay line for 1000 ms:


  // Drive LED_PIN to LOW (0V) — this turns the LED off.
  // ▶ Write the digitalWrite line to set LED_PIN LOW:


  // ▶ Print "LED: OFF" to the Serial Monitor:


  // Wait another 1000 ms before the loop repeats.
  // ▶ Write the second delay line:
...
// ▶ Declare a constant integer named LED_PIN and assign it 9:
//   (Pin 9 has the ~ symbol — it is PWM-capable. analogWrite() will not work on other pins.)
...
void loop() {
  // Read the potentiometer value. Returns an integer from 0 to 1023.
  // ▶ Read from POT_PIN and store it in an int named potValue:


  // Rescale the ADC range (0–1023) to the PWM range (0–255) using map().
  // Without this, any ADC value above 255 would clamp to full brightness.
  // Store the result in an int named pwmValue.
  // ▶ Write the map line:


  // Map the ADC value to a percentage (0–100) for human-readable display.
  // Store the result in an int named percent.
  // ▶ Write the map line for percent:


  // Send the PWM value to the LED pin.
  // analogWrite() automatically generates the correct duty cycle.
  // ▶ Write the analogWrite line using LED_PIN and pwmValue:


  // Print all three values on one line.
  Serial.print("       ");
  // ▶ Print potValue (no newline):

  Serial.print("        |      ");
  // ▶ Print pwmValue (no newline):
  ...
  // 100 ms delay — short enough that the LED responds quickly to knob movement.
  // ▶ Write the delay line:
...
// ▶ Declare POT_PIN as a constant integer set to A0:

// ▶ Declare LED_PIN as a constant integer set to 9:
...
void loop() {
  ...
  // ▶ Map potValue (0–1023) to PWM range (0–255) and store in int pwmValue:

  // ▶ Map potValue (0–1023) to percentage (0–100) and store in int percent:


  // ▶ Send pwmValue to LED_PIN using analogWrite():


  // Print the opening bracket of the progress bar.
  Serial.print("Brightness [");

  // Calculate how many '#' bars to print.
  // Each '#' represents 10% brightness. Dividing percent by 10 gives 0–10 bars.
  // Store the result in an int named bars.
  // ▶ Write the line: int bars = percent / 10;


  // Loop 10 times — once for each segment of the bar.
  // If the current segment index (i) is less than bars, print '#'. Otherwise print '-'.
  // The loop counter variable is i, starting at 0 and going up to (but not including) 10.
  // ▶ Write the for loop with its opening brace:

    // ▶ Inside the loop: write an if statement — if i < bars, print "#", else print "-":



  // Close the bar and print the percentage value.
  // These lines are already written for you.
  Serial.print("] ");
...
![Generic voltage divider schematic — two resistors R1 and R2 in series between Vin and GND, with Vout at their junction and the formula labeled](images/15-voltage%20divider%20schematics.jpg)

![LDR voltage divider schematic showing LDR as upper arm and 10 kΩ as lower arm, with junction connected to Arduino A1](images/16.%20LDR%20voltage%20divider%20schematic-a.jpeg)

![LDR voltage divider breadboard schematic — alternate view](images/16.%20LDR%20voltage%20divider%20schematic-b.png)
...
![LDR voltage divider and LED wiring on breadboard — full circuit for Step 4](images/17.%20LDR%20+%20LED%20wiring%20diagram%20(breadboard).png)
...
// We give the analog pin a name so the code is easier to read.
// LDR voltage divider output is connected to pin A1.
// A0 is kept free for the potentiometer from Step 2.
const int LDR_PIN = A1;

void setup() {
  // Initialize serial communication so we can see output in the Serial Monitor.
  // The number inside Serial.begin() is the baud rate — must match the Serial Monitor dropdown.
  // ▶ Write the line to begin serial at 9600 baud:


  // Print a header so the output is easy to read.
  // Use Serial.println() to print each line below:
...
void loop() {
  // Read the raw ADC value from the LDR pin.
  // analogRead() takes a pin name and returns an integer between 0 and 1023.
  // Store the result in an int variable called ldrRaw.
  // ▶ Write the line to read from LDR_PIN and store it in ldrRaw:


  // Convert the ADC value to voltage using the formula: V = ADC × (5.0 / 1023.0)
  // Use a float variable called voltage to store the result.
  // Remember: write 5.0 and 1023.0, not 5 and 1023 — otherwise integer division gives 0.
  // ▶ Write the conversion line:


  // Print the ADC value and voltage on the same line, separated by " | "
  // Use Serial.print() for everything except the last value — use Serial.println() to end the line.
  Serial.print("  ");
  Serial.print(ldrRaw);
  Serial.print("  |  ");
  // ▶ Print voltage with 3 decimal places, followed by " V", ending the line:


  // Wait 400 ms before the next reading.
  // ▶ Write the delay line:
...
// ▶ Declare a constant integer named LDR_PIN and assign it A1:


// These two variables track the lowest and highest ADC readings seen so far.
// They are declared outside loop() so they persist between loop iterations.
// minVal starts at 1023 (the maximum possible) so any real reading will be lower.
// maxVal starts at 0 (the minimum possible) so any real reading will be higher.
int minVal = 1023;
int maxVal = 0;
...
void loop() {
  ...
  // ▶ Read from LDR_PIN and store in an int named ldrRaw:


  // Update minVal if the current reading is lower than what we have seen before.
  // This uses an if statement — if ldrRaw is less than minVal, update minVal.
  // ▶ Write the if statement to update minVal:


  // Update maxVal if the current reading is higher than what we have seen before.
  // ▶ Write the if statement to update maxVal:


  // Calculate the midpoint between the darkest and brightest reading seen.
  // This midpoint is our suggested threshold.
  // Store it in an int named threshold.
  // ▶ Write the line: int threshold = (minVal + maxVal) / 2;


  // Print all four values on one line — already written for you.
  Serial.print("Now: ");
  Serial.print(ldrRaw);
  Serial.print("  Min: ");
  Serial.print(minVal);
...
// Replace 400 below with YOUR calibrated value from Task 4.2.
// This is not a magic number — it is specific to your sensor, resistor, and room.
// ▶ Declare a constant integer named THRESHOLD and assign it your measured midpoint:
...
void loop() {
  // Read the current LDR ADC value.
  // ▶ Read from LDR_PIN and store it in an int named ldrRaw:


  // Decide LED state based on the reading versus the threshold.
  // Low ADC = low V_out = high R_LDR = darkness → LED on.
  // High ADC = high V_out = low R_LDR = bright → LED off.
  // Use an if/else structure. The condition is: ldrRaw < THRESHOLD
  // ▶ Write the if statement opening:

    // Inside the if block (dark condition):
    // Turn the LED on using digitalWrite(LED_PIN, HIGH).
    // ▶ Write the digitalWrite line:

    // Print the LDR reading and state. Already written for you.
    Serial.print("   ");
    Serial.print(ldrRaw);
    Serial.println("     |   ON        |  Dark");

  // ▶ Write the else block opening:

    // Inside the else block (bright condition):
    // Turn the LED off using digitalWrite(LED_PIN, LOW).
    // ▶ Write the digitalWrite line:

    Serial.print("   ");
    Serial.print(ldrRaw);
    Serial.println("     |   OFF       |  Bright");
  // ▶ Close the else block with a closing brace:
...
![Contact bounce oscilloscope waveform — ideal clean signal vs real bouncing signal in the 1–20 ms window after a button press](images/22-button%20contact%20bounce%20oscilloscope%20waveform%20diagram.jpg)
...
// ▶ Declare a constant integer named BUTTON_PIN and assign it 4:


void setup() {
  // Configure BUTTON_PIN as a plain INPUT — no pull-up, no pull-down.
  // When the button is open, the pin is floating and has no defined voltage.
  // ▶ Write the pinMode line using BUTTON_PIN and INPUT:
...
void loop() {
  ...
  // Store the result in an int named reading.
  // ▶ Write the digitalRead line:


  // Print "Pin 4: " followed by "HIGH" or "LOW" depending on the reading.
  // Use the ternary operator: (condition ? "value if true" : "value if false")
  // The line is already written for you — observe what it does.
  Serial.print("Pin 4: ");
...
void setup() {
  // Configure as plain INPUT — the pull-down is provided by the external 10 kΩ resistor,
  // not by the Arduino's internal pull-up. Do NOT use INPUT_PULLUP here.
  // ▶ Write the pinMode line using BUTTON_PIN and INPUT:
...
void loop() {
  ...
  // ▶ Read BUTTON_PIN using digitalRead() and store it in an int named reading:


  // Print the reading and its meaning.
  // With a pull-down: HIGH means pressed, LOW means released.
  // The if/else block below is already written — study its logic.
  Serial.print("    ");
  if (reading == HIGH) {
...
void setup() {
  // Activate the internal pull-up resistor on BUTTON_PIN.
  // This is done by passing INPUT_PULLUP (not INPUT) to pinMode().
  // The pin now rests at HIGH automatically — no external resistor needed.
  // ▶ Write the pinMode line using BUTTON_PIN and INPUT_PULLUP:
...
void loop() {
  ...
  // ▶ Read BUTTON_PIN and store in an int named reading:


  // Print the reading and its meaning.
  // With INPUT_PULLUP: LOW means pressed, HIGH means released — opposite of Task 5.2!
  // The if/else below checks for LOW first because that is the pressed state.
  Serial.print("    ");
  if (reading == LOW) {
...
![Button and LED wiring on breadboard — button leg to Pin 4, leg to GND; LED with 220 Ω on Pin 9](images/23.%20Button%20+%20LED%20wiring%20diagram%20(breadboard).png)
...
// ▶ Declare BUTTON_PIN as a constant integer set to 4:

// ▶ Declare LED_PIN as a constant integer set to 9:


// These variables must be declared outside loop() so they retain their values
// between iterations. 'unsigned long' is used for millis() timestamps because
// millis() can return very large numbers that don't fit in a regular int.
...
// bool is a true/false variable type. ledOn tracks whether the LED is currently on.
bool ledOn = false;
...
void setup() {
  ...
  // Start the LED in the OFF state.
...
void loop() {
  // Step 1: Read the raw current state of the button pin.
  // ▶ Read BUTTON_PIN and store in an int named currentReading:


  // Step 2: If the raw reading has changed since the last loop iteration,
  // record the current time. We don't trust the change yet — it might be bounce.
  // millis() returns the time in milliseconds since the Arduino powered on.
  // ▶ Write the if statement: if currentReading != lastRawReading, set lastChangeTime = millis():


  // Step 3: Check if the pin has been stable for at least DEBOUNCE_MS milliseconds.
  // (millis() - lastChangeTime) gives the time elapsed since the last change.
  // If that duration is greater than DEBOUNCE_MS, the reading is stable.
  // ▶ Write the if statement checking (millis() - lastChangeTime) > DEBOUNCE_MS:

    // Step 4: Inside the stable-state block — check if the confirmed state has changed.
    // If currentReading differs from confirmedButtonState, this is a new stable state.
    // ▶ Write the if statement: if currentReading != confirmedButtonState:

      // Accept the new state as confirmed.
      // ▶ Write: confirmedButtonState = currentReading;


      // Step 5: With INPUT_PULLUP, LOW means the button is pressed.
      // We only toggle on press (LOW), not on release — otherwise each press
      // would toggle twice (once when pressed, once when released).
      // ▶ Write the if statement checking confirmedButtonState == LOW:

        // Flip the LED state using the NOT operator (!).
        // ledOn = !ledOn means: if ledOn was true, it becomes false, and vice versa.
        // ▶ Write: ledOn = !ledOn;


        // Drive the LED pin based on the new ledOn value.
        // Use a ternary: ledOn ? HIGH : LOW
        // ▶ Write the digitalWrite line:
    ...
    // ▶ Close the confirmed-state-changed if block:

  // ▶ Close the stable-state if block:


  // Step 6: Update lastRawReading for comparison on the next loop iteration.
  // This MUST be at the very end of loop(), outside all if blocks.
  // If it were inside an if block, it might not update every iteration,
  // and the debounce logic would break.
  // ▶ Write: lastRawReading = currentReading;
...
![LM35 output voltage vs temperature graph — linear relationship from 0°C (0 mV) to 100°C (1000 mV), slope labeled as 10 mV/°C](images/24.%20LM35%20output%20voltage%20vs%20temperature%20graph.png)
...
![Complete final circuit wiring on breadboard — all components: potentiometer on A0, LDR on A1, LM35 on A2, button on Pin 4, LED on Pin 9](images/25.%20Complete%20final%20circuit%20wiring%20diagram%20(breadboard).png)
...
// LM35 VOUT is connected to A2.
// A0 = potentiometer, A1 = LDR — kept from previous steps.
// ▶ Declare a constant integer named TEMP_PIN and assign it A2:
...
void loop() {
  // Read the raw ADC value from the LM35 pin.
  // ▶ Read from TEMP_PIN and store it in an int named rawADC:


  // Step 1: Convert ADC value to voltage.
  // Formula: V = ADC × (5.0 / 1023.0)
  // Use a float named voltage. Use 5.0 and 1023.0 to force floating-point division.
  // ▶ Write the voltage conversion line:


  // Step 2: Convert voltage to temperature in Celsius.
  // The LM35 outputs 10 mV per °C = 0.010 V per °C.
  // Dividing V by 0.010 is the same as multiplying V by 100.
  // Use a float named temperatureC.
  // ▶ Write the temperature conversion line (temperatureC = voltage * 100.0):


  // Print all three values — partially written for you.
  Serial.print("  ");
  // ▶ Print rawADC (no newline):

  Serial.print("  |    ");
  // ▶ Print voltage with 4 decimal places (no newline):

  Serial.print(" V    |    ");
  // ▶ Print temperatureC with 2 decimal places (no newline):

  // ▶ End the line by printing " °C" with a newline:


  // Read once per second — temperature changes slowly.
  // ▶ Delay 1000 ms:
...
// analogRead() can be called on different pins in sequence within the same loop().
...
// ▶ Declare LDR_PIN as a constant integer set to A1:

// ▶ Declare TEMP_PIN as a constant integer set to A2:
...
void loop() {
  ...
  // ▶ Read POT_PIN and store in an int named potRaw:

  // ▶ Read LDR_PIN and store in an int named ldrRaw:

  // ▶ Read TEMP_PIN and store in an int named tempRaw:


  // Convert temperature using the combined formula: ADC × (5.0 / 1023.0) × 100.0
  // Store in a float named tempC.
  // ▶ Write the temperature conversion line:


  // Control LED brightness from the potentiometer (same as Task 3.2).
  // Map potRaw (0–1023) to PWM range (0–255) and store in an int named pwmValue.
  // ▶ Write the map line for pwmValue:

  // ▶ Write the analogWrite line to set LED_PIN brightness:


  // Map potRaw to percentage (0–100) for display. Store in int potPercent.
  // ▶ Write the map line for potPercent:


  // Print all values on one line — partially written for you.
  Serial.print("  ");
  // ▶ Print potPercent (no newline):

  Serial.print("%   |  ");
  // ▶ Print ldrRaw (no newline):

  Serial.print("   |    ");
  // ▶ Print tempC with 1 decimal place (no newline):

  Serial.print(" °C   |    ");
  // ▶ Print pwmValue (no newline):

  // ▶ End the line by printing " PWM" with a newline:
...
// Prints a warning whenever temperature exceeds TEMP_HIGH or falls below TEMP_LOW.
// Named constants make thresholds easy to find and change without hunting
// through the code for a magic number.

// ▶ Declare TEMP_PIN as a constant integer set to A2:

// ▶ Declare LED_PIN as a constant integer set to 9:

// These float constants define the alert boundaries.
// To test: set TEMP_HIGH to 1°C below your current room temperature.
// Change these values to match your testing environment.
...
void setup() {
  ...
  // Print the threshold values so the user knows what limits are active.
  // These lines are already written for you.
...
void loop() {
  // Read raw ADC from the LM35.
  // ▶ Read from TEMP_PIN and store in an int named rawADC:


  // Convert to temperature using the combined formula.
  // ▶ Write: float tempC = rawADC * (5.0 / 1023.0) * 100.0;


  // Print the temperature value first, before the alert message.
  Serial.print("Temp: ");
  // ▶ Print tempC with 1 decimal place (no newline):

  Serial.print(" °C  ");

  // Check which zone the temperature falls into and respond accordingly.
  // Use if / else if / else to cover three cases:
  //   1. tempC > TEMP_HIGH  →  turn LED on, print high temp warning
  //   2. tempC < TEMP_LOW   →  turn LED off, print low temp warning
  //   3. else               →  normal range, print OK
  // ▶ Write the if statement for the high temperature case:

    // Turn the LED on as a visual alarm.
    // ▶ Write the digitalWrite line to set LED_PIN HIGH:

    Serial.print("  [!] HIGH TEMP WARNING");

  // ▶ Write the else if statement for the low temperature case:
  ...
  // ▶ Write the else block for the normal range:

    // Temperature is within normal range — no action needed on the LED.
